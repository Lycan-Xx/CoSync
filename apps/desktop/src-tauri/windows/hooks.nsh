...
!macro NSIS_HOOK_PREINSTALL
  ...
  StrCpy $0 1
  ExecWait '"$SYSDIR\netsh.exe" advfirewall firewall set rule name="CoSync Pairing (UDP 48215)" new dir=in action=allow program="$INSTDIR\desktop.exe" protocol=UDP localport=48215 profile=private remoteip=localsubnet enable=yes' $0
  ${If} $0 != 0
  ...
  ${If} $0 != 0
    ...
    MessageBox MB_ICONSTOP|MB_OK "CoSync could not configure Windows Firewall for local pairing.$\r$\n$\r$\nSetup cannot continue because pairing would be blocked. Please run the installer as an administrator." /SD IDOK
    Abort
  ${EndIf}
!macroend
