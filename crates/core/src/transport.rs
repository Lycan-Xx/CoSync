...
use crate::verifier::{
    AcceptAnyClientVerifier, PinnedClientVerifier, PinnedServerVerifier, TrustedClientFingerprints,
    TrustedClientsVerifier,
};
...
///
/// This fixed-pin form is useful when exactly one peer is expected. Desktop
/// hosts that accept several paired devices use
/// [`build_trusted_server_endpoint`] and its runtime-updatable trust set.
...
/// Build the desktop's steady-state endpoint. Unlike the one-time pairing
/// endpoint, TLS handshakes here succeed only for fingerprints already in the
/// persistent paired-device store. The shared trust set can be updated after a
/// successful pairing without rebinding the listener.
pub fn build_trusted_server_endpoint(
    bind_addr: SocketAddr,
    cert: &DeviceCertificate,
    ...
    let mut tls_config = rustls::ServerConfig::builder()
        .with_safe_defaults()
        .with_client_cert_verifier(TrustedClientsVerifier::new(trusted))
        .with_single_cert(vec![cert.rustls_certificate()], cert.rustls_private_key())?;
    tls_config.alpn_protocols = vec![b"cosync".to_vec()];
...
/// [`build_trusted_server_endpoint`] with persisted fingerprint pins.
...
impl Session {
    ...
    /// The certificate check already happened during the handshake (via a
    /// pinned client verifier) by the time this returns `Ok`.
